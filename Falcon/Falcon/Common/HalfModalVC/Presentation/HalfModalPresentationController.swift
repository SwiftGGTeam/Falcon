...
class HalfModalPresentationController: UIPresentationController {
    ...
    // TODO: - An elegant way to set configs.
    /// top distance between top margin and presentationController
    public var topGap = UIApplication.shared.statusBarFrame.size.height + 20
    /// background scale factor. (1, 1) means zooming is disabled. default is (0.9, 0.9).
    public var scaleFactor = CGAffineTransform(scaleX: 1, y: 1)
    /// is full screen size allowed. default is true.
    public var allowFullScreen = false
...
//        blurEffectView.contentView.addSubview(vibrancyEffectView)
    ...
    @objc func onPan(pan: UIPanGestureRecognizer) -> Void {
        ...
        case .changed:
            ...
            switch state {
            case .normal:
                presentedView!.frame.origin.y = endPoint.y + topGap
            case .adjustedOnce:
    ...
    func changeScale(to state: ModalScaleState) {
        if let presentedView = presentedView, let containerView = self.containerView {
            UIView.animate(withDuration: 0.8, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.5, options: .curveEaseInOut, animations: { () -> Void in
                ...
                let halfFrame = CGRect(origin: CGPoint(x: 0, y: self.topGap),
                                       size: CGSize(width: containerFrame.width, height: containerFrame.height - self.topGap))
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        return CGRect(x: 0, y: topGap, width: containerView!.bounds.width, height: containerView!.bounds.height - topGap)
    }
    ...
    override func presentationTransitionWillBegin() {
        ...
        if let containerView = self.containerView, let coordinator = presentingViewController.transitionCoordinator {
            ...
            coordinator.animate(alongsideTransition: { (context) -> Void in
                dimmedView.alpha = 1
                self.presentingViewController.view.transform = self.scaleFactor
            }, completion: nil)
        }
    }
